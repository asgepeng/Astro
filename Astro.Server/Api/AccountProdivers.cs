...
namespace Astro.Server.Api
{
    internal static class AccountProdivers
    {
        ...
        {
            if (Application.IsWinformApp(context.Request)) return Results.File(await db.GetAccountProviderTableAsync(), "application/octet-stream");
            return Results.Ok();
        }
        private static async Task<IResult> GetByIdAsync(short id, IDatabase db, HttpContext context)
        {
            if (Application.IsWinformApp(context.Request)) return Results.File(await db.GetAccountProviderAsync(id), "application/octet-stream");
            return Results.Ok();
        }
